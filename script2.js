...
const reasoningModeBadge = document.getElementById("reasoningModeBadge");
...
// GitHub Pages cannot run SWI-Prolog, so the site will automatically use the
// frontend simulation if this local backend is unavailable.
const USE_PROLOG_BACKEND = true;
...
function setReasoningMode(mode) {
  if (!reasoningModeBadge) {
    return;
  }

  const isBackend = mode === "backend";
  reasoningModeBadge.classList.toggle("simulation", !isBackend);
  reasoningModeBadge.textContent = isBackend
    ? "Reasoning Mode: Prolog Backend (Local)"
    : "Reasoning Mode: Frontend Simulation";
}

function buildPrologPayload(assessment) {
  return {
    person: activeScenario || "custom",
    stress: assessment.stressRisk,
    urgency: assessment.urgencyRisk,
    support: assessment.supportRisk,
    isolation: assessment.isolationRisk,
    sleepRisk: assessment.sleepRisk,
    moodRisk: assessment.energyRisk,
    heartRate: assessment.iot.heartRate,
    sleepHours: assessment.iot.sleepHours,
    activity: String(assessment.iot.activity || "medium").toLowerCase()
  };
}

function normalizeBackendCategory(category, fallbackCategory) {
  const normalized = String(category || fallbackCategory || "medium").toLowerCase();
  return fuzzyRiskProfiles[normalized] ? normalized : fallbackCategory || "medium";
}

function strategyLabelFromBackend(strategy, fallbackLabel) {
  const strategyMap = {
    immediate_support: "Immediate Support",
    guided_support: "Guided Support",
    general_support: "General Support"
  };
  return strategyMap[String(strategy || "").toLowerCase()] || fallbackLabel;
}

function numberFromBackend(value, fallbackValue) {
  const parsed = Number(value);
  return Number.isFinite(parsed) ? parsed : fallbackValue;
}

function assessmentFromPrologBackend(result, fallbackAssessment) {
  const fuzzy = result?.fuzzy || {};
  const backendStrategy = result?.strategy || {};
  const backendIot = result?.iot || {};
  const categoryKey = normalizeBackendCategory(fuzzy.category, fallbackAssessment.categoryKey);
  const profile = fuzzyRiskProfiles[categoryKey] || fuzzyRiskProfiles.medium;
  const risk = Number(numberFromBackend(fuzzy.riskScore, fallbackAssessment.risk).toFixed(2));
  const iotRiskValue = Number(numberFromBackend(backendIot.iotRisk, fallbackAssessment.iotRisk).toFixed(2));
  const iot = {
    ...fallbackAssessment.iot,
    heartRate: numberFromBackend(backendIot.heartRate, fallbackAssessment.iot.heartRate),
    sleepHours: numberFromBackend(backendIot.sleepHours, fallbackAssessment.iot.sleepHours),
    activity: backendIot.activity || fallbackAssessment.iot.activity,
    status: fallbackAssessment.iot.status,
    risk: iotRiskValue
  };
  const emotionalState = categoryKey === "high" && risk >= 0.85 ? profile.urgentState : profile.emotionalState;
  ...
  return {
    ...
    queue: fuzzy.queue || profile.queue,
    wait: fuzzy.waitTime || profile.wait,
    position: fuzzy.position || profile.position,
    action: fuzzy.action || profile.action,
    plan: result?.agents?.planningAgent || profile.plan,
    strategy: strategyLabelFromBackend(backendStrategy.selected, profile.strategy),
    utility: Number.isFinite(Number(backendStrategy.utility)) ? `${backendStrategy.utility} utility` : profile.utility,
    emotionalState,
    dashboardStatus: profile.dashboardStatus,
    dashboardBar: profile.dashboardBar,
    iot,
    iotRisk: iotRiskValue,
    backend: result
  };
}

async function resolveAssessmentForRun() {
  const frontendAssessment = calculateFuzzyAssessment();

  if (USE_PROLOG_BACKEND && typeof window.assessWithPrologBackend === "function") {
    const backendResult = await window.assessWithPrologBackend(buildPrologPayload(frontendAssessment));

    if (backendResult) {
      setReasoningMode("backend");
      return assessmentFromPrologBackend(backendResult, frontendAssessment);
    }

    console.warn("Prolog backend not available, using frontend simulation.");
  }

  setReasoningMode("frontend");
...
function updateAgentFlow(assessment) {
  const backendAgents = assessment.backend?.agents;

  if (agentDetection) {
    agentDetection.textContent = backendAgents?.detectionAgent || `Risk detected = ${assessment.risk.toFixed(2)}`;
  }
  if (agentDecision) {
    agentDecision.textContent = backendAgents?.decisionAgent || `Category = ${assessment.shortLabel}`;
  }
  if (agentPlanning) {
    agentPlanning.textContent = backendAgents?.planningAgent || assessment.plan;
  }
  if (agentSupport) {
    agentSupport.textContent = backendAgents?.supportAgent || `${assessment.queue} · ${assessment.position}`;
  }
}

function updateMinimaxStrategy(assessment) {
  const backendStrategy = assessment.backend?.strategy;

  if (minimaxStrategy) {
    minimaxStrategy.textContent = `Selected strategy: ${assessment.strategy} (${assessment.utility})`;
  }
  if (minimaxReason) {
    minimaxReason.textContent = backendStrategy?.reason || "Selected because this strategy gives the best support utility for the current risk level.";
  }
}
...
function renderReasoning(assessment) {
  ...
  const backendXai = assessment.backend?.xai;
  ...
  reasonFuzzy.textContent = backendXai?.fuzzyStep || minText;
  reasonFinal.textContent = backendXai?.finalStep || maxText;
  ...
  if (reasonExplain) {
    reasonExplain.textContent = backendXai?.decisionReason || `Based on the given stress, urgency, support, social isolation, sleep, mood, and IoT signals, the system assigned the user to ${assessment.queue} at ${assessment.position} because the final risk score is ${assessment.risk.toFixed(2)}.`;
  }
}
...
async function runFuzzyAssessment() {
  if (!runFuzzyAssessmentBtn || !fuzzyLoadingCard || !fuzzyProgressFill) {
    renderFuzzyResult(await resolveAssessmentForRun(), { animate: true });
    return;
  }

  const assessmentPromise = resolveAssessmentForRun();
  const duration = 2600;
